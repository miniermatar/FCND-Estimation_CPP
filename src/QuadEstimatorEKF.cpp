...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  Quaternion<float> imu_attitude= Quaternion<float>::FromEuler123_RPY(rollEst,pitchEst,ekfState(6)); //converting Euler angles to quaternion
  V3D gyro_v3d(gyro); // converting from V3F to V3D
  imu_attitude.IntegrateBodyRate(gyro_v3d,dtIMU); //Integrating gyro data

  float predictedPitch = imu_attitude.Pitch();//pitchEst + dtIMU * gyro.y;
  float predictedRoll = imu_attitude.Roll();//rollEst + dtIMU * gyro.x;
  ekfState(6) = imu_attitude.Yaw();//ekfState(6) + dtIMU * gyro.z;	// yaw

  // normalize yaw to -pi .. pi
  if (ekfState(6) > M_PI) ekfState(6) -= 2.0f*M_PI;
  if (ekfState(6) < -M_PI) ekfState(6) += 2.0f*M_PI;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  V3F accel_inertial_frame=attitude.Rotate_BtoI(accel); //converting acceleration from the body frame to the inertial frame
  //Integrating velocities to get change in position
  predictedState(0)=curState(3)*dt + predictedState(0);
  predictedState(1)=curState(4)*dt + predictedState(1);
  predictedState(2)=curState(5)*dt + predictedState(2);
  //Integrating acceleration to get change in position
  predictedState(3)=accel_inertial_frame.x*dt + predictedState(3);
  predictedState(4)=accel_inertial_frame.y*dt + predictedState(4);
  predictedState(5)=(accel_inertial_frame.z-CONST_GRAVITY)*dt + predictedState(5); //accounting for acceleration due to gravity
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
// Rgbprime equation obtained from quadrotor estimation paper, equation 52
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
 // identity matrix is already set. Adding values based on equation 51 from estimation for quadrotor paper
  gPrime(0,3)=dt;
  gPrime(1,4)=dt;
  gPrime(2,5)=dt;
  gPrime(3,6)=dt*(accel*RbgPrime(0)).sum();
  gPrime(4,6)=dt*(accel*RbgPrime(1)).sum();
  gPrime(5,6)=dt*(accel*RbgPrime(2)).sum();
  ekfCov=gPrime*ekfCov*gPrime.transpose() + Q; // from Algorithm 2 table in estimation for quadrotors, equation 4
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  for (int i=0; i<6; i++) {
      zFromX(i)=ekfState(i); // updating zFromX to match the ekfstate, excluding yaw (ekfstate(6))
      hPrime(i,i)=1; // identity matrix based on equation 55 in section 7.3.1 in Estimation for Quadrotors
  }
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  zFromX(0)=ekfState(6);
  hPrime(6)=1; // Jacobian based on magnometer. As explain in section 7.3.2 (Equation 58) in estimation for quadrotors
  VectorXf yaw_difference=z-zFromX;

  if (yaw_difference(0)<-M_PI) {
      z(0)=z(0)+2.0f*M_PI;
  }
  else if (yaw_difference(0)>M_PI) {
      z(0)=z(0)-2.0f*M_PI;
  }
